...
export const actions = {
	create: async ({cookies, request, params}) => {
		...
		if( photoType !== 'profile' || !photoIndex ){
		...
		const photoUrl = `${PUBLIC_SERVER}/companies/${companyId}/photo?index=${photoIndex}`
	...
	update: async ({cookies, request, params}) => {
		...
		const photoUrl = `${PUBLIC_SERVER}/companies/${companiesId}/photo?index=${photoIndex}`
		...
		throw redirect(302, `/companies/${params.id}`)
	},
